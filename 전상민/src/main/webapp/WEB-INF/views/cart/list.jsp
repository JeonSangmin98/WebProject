...
<main id="main" class="main">
			...
			<div class="card">
				...
				<div class="card-body">
					<div class="form-check">
						<input class="form-check-input checkAll" type="checkbox" id="invalidCheck">
						<label class="form-check-label" for="invalidCheck">전체 선택</label>
					</div>
		...
		<tbody>
			<c:forEach items="${cartList}" var="cart">
				<tr class="table-light">
					<td class="cartTotal">
						<input type="checkbox" class="form-check-input checkCart" checked="checked">
						<input type="hidden" class="hiddenCartTotalPrice" value="${cart.totalPrice}">
						<input type="hidden" class="hiddenCartCount" value="${cart.cartCount}">
						<input type="hidden" class="hiddenPrice" value="${cart.price}">
						<input type="hidden" class="hiddenbookCount" value="${cart.bookCount}"> 
						<input type="hidden" class="hiddenPoint" value="${cart.point}"> 
						<input type="hidden" class="hiddenTotalPoint" value="${cart.totalPoint}">
					...
					<td>
						<div class="numBtn">
							<button class="btn btn-outline-primary minusBtn" type="button"><i class="bi bi-dash-lg"></i></button>
							<input type="number" class="form-control countValue" value="${cart.cartCount}" name="cartCount" min="1">
							<button class="btn btn-outline-primary plusBtn" type="button"><i class="bi bi-plus-lg"></i></button>
						</div>
						<button class="btn btn-primary updateCountBtn" data-cartBno="${cart.cartBno}">변경</button>
					</td>
					<td>
						<button class="btn btn-danger deleteBtn" data-cartBno="${cart.cartBno}">삭제</button>
					</td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<!-- 주문 form -->
	<form action="/order/list/${member.memberId}" method="get" class="orderForm">
	</form>
...
function setCartTotal() {
	...
	$(".orderBtn").on("click",function() {
		if ($(".totalPrice").text() == undefined || $(".totalPrice").text() == 0) {
			alert("장바구니에 담긴 물품이 없습니다.");
		} else {
			let form_contents = '';
			let orderNumber = 0;
			$(".cartTotal").each(function(index, element) {
				if ($(element).find(".checkCart").is(":checked") === true) {
					let bno = $(element).find(".hiddenBookBno").val();
					console.log(bno);
					let bookCount = $(element).find(".hiddenCartCount").val();
